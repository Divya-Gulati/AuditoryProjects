...
fid =fopen(fullfile(folderName,'ASSR_exp1_longDur.txt'),'w');
...
for mf = modFreq
    for mi = modIndex
        str = sprintf('snd(Azi_0.0_Elev_0.0_Type_2_RF_1000.0_RP_0.0_MD_%2.1f _RV_%1.1f _Dur_1500.wav)',mi*100,mf);
        fprintf(fid,'\n%d\t%d\t%d\t%s\t%s',count,1,1,'timingfile_assr_exp1_longDur', str);
        count = count+1;
    end
end
