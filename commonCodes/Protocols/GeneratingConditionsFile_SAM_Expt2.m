...
fid =fopen(fullfile(folderName,'SAM_Expt2.txt'),'w');
    ...
    for mi2 = 1:length(modIndex2)
        for mf1 = 1:length(modFreq)
            for mf2 = 1:length(modFreq2)
                str = sprintf('snd(Azi_0.0_Elev_0.0_Type_3_RF_40.0_RP_%2.1f_MD_%2.1f _RV_%1.1f _Dur_800.wav)',modIndex2(mi2)*100,modIndex(mi1)*100,modFreq(mf1));
...
%%blank stimuli
fprintf(fid,'\n%d\t%d\t%d\t%s\t%s\t%s\t',count,1,1,'timingfile_auditory','snd(Azi_0.0_Elev_0.0_Type_0_RF_0.0_RP_0_MD_0.0_RV_0.0_Dur_800.wav)');
fclose(fid);
